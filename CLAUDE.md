...
**Ollama + Open WebUI are deployed**: Ollama running GPU-scheduled on `gpu-node-01`, Open WebUI on `control-plane-01`, both in the `ollama` namespace (`infra/k8s/base/ollama-openwebui/`). `gemma3:4b` pulled successfully. Reachable from the Dell G5 browser at `http://192.168.0.106:30080` (NodePort, per `docs/decisions/0009`) — Open WebUI's connection to Ollama was blocked by the DNS (`0010`) and asymmetric-firewall (`0011`) incidents above; both fixed. Still need to confirm Open WebUI can actually talk to Ollama end-to-end post-fix and that a GPU-backed chat response works.

**Also discovered**: `gpu-node-01` has a **pre-existing native `ollama.service`** (systemd, not Kubernetes-managed) running since before this session's work, bound to `127.0.0.1:11434` on the host, with its own models (`qwen2.5:14b`, `llama3.1:8b`, `batiai/qwen3.6-27b`) already pulled. Not yet reconciled with the new Kubernetes-managed Ollama deployment — decide whether to retire the native service, keep both, or migrate its models into the cluster's PVC.
