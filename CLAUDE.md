...
**ComfyUI: three problems with the first image, switched entirely**: `infra/k8s/base/comfyui/`, GPU-scheduled on `gpu-node-01` (same single-GPU constraint as Ollama — see that directory's README for the manual scale-up/down toggle between the two). First deploy of `ghcr.io/ai-dock/comfyui:latest-cuda` (1) auto-created **public internet tunnels** (Cloudflare Quick Tunnels) exposing Jupyter/syncthing/etc. with a default password of `"password"` — caught and scaled to zero within minutes; (2) its Docker/RunPod-specific port-advertisement automation didn't understand Kubernetes' own auto-injected Service env vars, so ComfyUI never bound anywhere reachable even after fixing (1); (3) its bundled PyTorch (2.4.1+cu121) doesn't support the RTX 5060 Ti's Blackwell architecture at all. Switched to `yanwk/comfyui-boot:cu130-slim-v2` (plain ComfyUI, no bundled extras, explicit Blackwell support). Full writeup in `docs/decisions/0012`. Redeploy with the new image not yet confirmed working.
...
- Redeploying ComfyUI with the fixed manifest (`docs/decisions/0012`) and confirming: no tunnel URLs on startup, GPU detected, image generation actually works
