...
**ComfyUI deployed, with a security incident along the way**: `infra/k8s/base/comfyui/`, GPU-scheduled on `gpu-node-01` (same single-GPU constraint as Ollama — see that directory's README for the manual scale-up/down toggle between the two). First deploy of `ghcr.io/ai-dock/comfyui:latest-cuda` auto-created **public internet tunnels** (Cloudflare Quick Tunnels) exposing Jupyter/syncthing/etc. with a default password of `"password"` — caught and scaled to zero within minutes. Full incident writeup, fix (env vars disabling all of it), and the reasoning for moving `WEB_PASSWORD` to a Kubernetes `Secret` instead of a committed literal (this repo is public) in `docs/decisions/0012`. Redeploy with the fixed manifest not yet confirmed clean.
