...
- Reconciling the pre-existing native `ollama.service` on `gpu-node-01` (found running outside Kubernetes, with its own already-pulled models) with the new Kubernetes-managed Ollama deployment — decide whether to retire it, keep both, or migrate its models
