...
- [ ] Configure containerd NVIDIA runtime on `gpu-node-01` (`infra/k8s/configure-nvidia-runtime.sh`)
- [ ] Label `gpu-node-01` and deploy the device plugin DaemonSet
- [ ] Validate `nvidia.com/gpu` shows as an allocatable resource on `gpu-node-01`
